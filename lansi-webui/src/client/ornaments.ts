...
const ornamentClasses: Record<OrnamentId, string> = {
  swordEmblem: css.ornamentSwordEmblem,
  redCrest: css.ornamentRedCrest,
  sword: css.ornamentSword,
  heart: css.ornamentHeart,
  flame: css.ornamentFlame,
  redOrb: css.ornamentRedOrb,
}
...
function targetFor(id: OrnamentId, targets: Targets): HTMLElement | null {
  switch (id) {
    case 'redCrest': return targets.selectedNav
    case 'swordEmblem': return targets.workspaceLabel ?? targets.tree
    case 'sword':
    case 'heart': return targets.composer
    case 'flame': return targets.dialog ?? targets.heading
    case 'redOrb': return targets.mascot
  }
}
