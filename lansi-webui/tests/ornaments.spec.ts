...
describe('ornament controller', () => {
  it('syncs idempotently, reacts to semantic targets, and disposes owned DOM', async () => {
    ...
    expect(document.querySelector('[data-dsh-ornament="sword"]')).not.toBeNull()
    ...
    await tick()
    expect(document.querySelector('[data-dsh-ornament="sword"]')).toBeNull()
    expect(document.querySelector('[data-dsh-ornament="heart"]')).not.toBeNull()

    const dialog = document.createElement('section')
    dialog.setAttribute('role', 'dialog')
    document.body.append(dialog)
    await tick()
    expect(document.querySelector('[data-dsh-ornament="flame"]')).not.toBeNull()
    expect(document.querySelectorAll('img[data-dsh-ornament]')).toHaveLength(5)

    const before = document.querySelector<HTMLImageElement>('[data-dsh-ornament="redCrest"]')!.src
    controller.setMode('dark')
    expect(document.querySelector<HTMLImageElement>('[data-dsh-ornament="redCrest"]')!.src).not.toBe(before)
  ...
  it('keeps only the narrow-screen ornament set', () => {
    fixture()
    const dialog = document.createElement('section')
    dialog.setAttribute('role', 'dialog')
    document.body.append(dialog)
  ...
  it('anchors the sword emblem to the workspace label immediately before the tree area', () => {
    document.body.innerHTML = `
      <section>
    ...
    expect(document.querySelector('#workspace-label')?.getAttribute('data-dsh-ornament-target')).toBe('swordEmblem')
